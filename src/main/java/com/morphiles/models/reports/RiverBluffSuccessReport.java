 ...
 */
public class RiverBluffSuccessReport extends Report {

    public JFreeChart chart;

    public RiverBluffSuccessReport(String reportName, PokerDataModel model){
        super();
        setReportName(reportName);
        this.setLayout(new BorderLayout());
        setModel(model);
        setChartTitle("River Bluffs BET on flop, CHECK turn, BET River across all players & hands");
    ...
    public void runReport(){
        ...
        for (int i=0; i<getModel().getRowCount(); i++){
            ...
            if (!isFMvBet && !isTMvBet && isRMvBet && !isRMvFolded)
