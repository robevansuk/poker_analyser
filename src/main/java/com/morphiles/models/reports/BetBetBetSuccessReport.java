 ...
 */
public class BetBetBetSuccessReport extends Report {

    public JFreeChart chart;

    public BetBetBetSuccessReport(String reportName, PokerDataModel model){
        super();
        setReportName(reportName);
        this.setLayout(new BorderLayout());
        setModel(model);
        setChartTitle("BET on flop, BET on turn, BET on river (all players & hands)");
    ...
    public void runReport(){
        ...
        for (int i=0; i<getModel().getRowCount(); i++){
            ...
            boolean isTMvBet = ((String)getModel().getValueAt(i, getColumnNamed("T Mv"))).contains("bet");
            boolean isRMvBet = ((String)getModel().getValueAt(i, getColumnNamed("R Mv"))).contains("bet");
            boolean isRMvFold = ((String)getModel().getValueAt(i, getColumnNamed("R Mv"))).contains("fold");
...
//            }
            ...
            if (isFMvBet && isTMvBet && isRMvBet && !isRMvFold) //&& isRMvNotShownBecausePotWasWon)// && isBetSizeInRange)
