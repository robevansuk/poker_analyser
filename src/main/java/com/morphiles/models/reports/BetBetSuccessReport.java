 ...
 */
public class BetBetSuccessReport extends Report {

    public JFreeChart chart;

    public BetBetSuccessReport(String reportName, PokerDataModel model){
        super();
        setReportName(reportName);
        this.setLayout(new BorderLayout());
        setModel(model);
        setChartTitle("BET on flop, BET on turn (all players & hands)");
    ...
    public void runReport(){
        ...
        for (int i=0; i<getModel().getRowCount(); i++){
            ...
            boolean isTMvFold = ((String)getModel().getValueAt(i, getColumnNamed("T Mv"))).contains("fold");
...
//            }
            ...
            if (isFMvBet && isTMvBet && !isTMvFold && !isRMvFold) //&& isRMvNotShownBecausePotWasWon)// && isBetSizeInRange)
